...
scale_cfr <- function(data_1_in, delay_fun, f){
  ...
  for(ii in 1:nrow(data_1_in)){
    ...
    for(jj in 0:(ii - 1)){
      ...
      known_jj <- (case_incidence[ii - jj]*f[[jj+1]])
...
output_cfr_timeseries <- function(main_data_in, delay_fun, f){
  ...
  {
    store_cfr <- rbind(store_cfr,scale_cfr(data_1[1:tt,], delay_fun, f))
  }
...
calculate_CIs_nCFR <- function(data_1, delay_dist)
{
  ...
  for(i in 1:nrow(data_1))
  {
    ...
    else if(data_1[i,]$cases < data_1[i,]$deaths)
    {
      ...
      CI_nCFR_raw_low[i]  <- signif(bin_conf(data_1[i,]$cases, data_1[i,]$deaths)[2]*100, 3)
...
nCFR_with_CIs_main_time_series <- function(data_1, delay_dist, f)
{
  ...
  output_cfr_timeseries(data_1, delay_dist, f)  -> CFRVariablename1
...
# function to calculate the timeseries of CFR values with CIs
cCFR_with_CIs_main_time_series <- function(data_1, delay_dist, f)
{
...
computenCFRTimeSeries <- function(data_in, delay_dist, f)
{
  ...
  currentnCFRTimeSeries <- nCFR_with_CIs_main_time_series(data_in_2, delay_dist, f)
  
}

computecCFRTimeSeries <- function(data_in,  delay_dist, f)
{
  ...
  currentcCFRTimeSeries <- cCFR_with_CIs_main_time_series(data_in_2, delay_dist, f)
}
