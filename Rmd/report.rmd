...
$$
where $c_{t}$ is the daily case incidence at time $t$, $f_t$ is a sample from the delay distribution between onset or hospitalisation and death. $u_t$ represents the underestimation of the known outcomes [@nishiura2009early] and is used as the denominator in the overall calculation of the corrected CFR (cCFR) instead of raw case numbers. This typically leads to an increase between the nCFR and the cCFR, as it predicts the outcome of a number of the current cases within the calculation. 
...
```{r fig_Main, echo=FALSE, fig.align='center', fig.cap="_Figure 1: The distributions along with the case and death data used to calculate the cCFR estimates. Panel A: the delay distributions of hospitalisation-to-death in cases with fatal outcomes (Top panel) - both are Lognormal distribution fits taken from Linton et al. A). Non-truncated distribution with a  mean of 14.5 days and SD of 6.7 days. B). Right-truncated distribution withj a mean of 20.2 days and SD of 11.6 day. Panels C and D: the case and death timeseries (respectively)._", out.width = '100%'}
